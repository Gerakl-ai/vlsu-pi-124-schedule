...
function inferSpace(text: string, kind: NoteKind, hasSubject: boolean) {
...
export function classifyNote(text: string, subjects: SubjectOption[]): NoteClassification {
  const normalized = normalizeNoteText(text);
  const subject = matchSubject(normalized, subjects);
  const kind = inferKind(normalized);
  const space = inferSpace(normalized, kind, Boolean(subject));
